...
public class LinkedListMitIteratorUndComparable<T extends Comparable<T>> implements ILinkedList<T>,Iterable<T> {
    ...
    public void addValue(T _value) {
        ...
        // Fall: Die Liste hat mindestens einen Wert
        ListElement<T> newElement = new ListElement<>(_value, null);

        // Wenn der neue Wert kleiner als das erste Element ist, wird er am Anfang eingefügt
        if (_value.compareTo(Front.getValue()) > 0) {
            newElement.Next = Front;
            Front = newElement;
            return;
        }

        // Durch die Liste iterieren, um die richtige Position für den neuen Wert zu finden mit einem Durchlaufelement
        ListElement<T> Durchzähler = Front;

        // Suche nach der richtigen Stelle, um den neuen Wert einzufügen
        while (Durchzähler.getNext() != null) {
            T nextValue =  (T) Durchzähler.getNext().getValue();
            if (_value.compareTo(nextValue) > 0) {
                break;
            }
            Durchzähler = Durchzähler.getNext();
        }

            // Füge das neue Element an der richtigen Stelle ein, da der While loop genau dort gestoppt hat wo wir das Element einfügen wollen
        newElement.Next = Durchzähler.getNext();
        Durchzähler.Next = newElement;

        // Wenn das neue Element am Ende eingefügt wurde, setze Back auf das neue Element
    ...
    private class CustomIteratorLinkedList implements Iterator<T>{
        ...
        public boolean hasNext() {
            if (Durchzähler == Front && Durchzähler == null) {
                return false;
            }
